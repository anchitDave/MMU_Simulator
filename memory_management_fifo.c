...
#define PT_MAX_LRU_COUNTER   650000000
...
/****************************************
* Linked List implementation FIFO
*****************************************/
...
static int number_of_queue_entries = 0;

//Track of head and tail of linked list for Queue
struct node *head = NULL;
struct node *tail = NULL;


void updateQueue(int frame) {

    struct node *current_node = head;
    struct node *prev_node = NULL;

    while (current_node != NULL && current_node->data != frame)
    {
    ...
    if (current_node != NULL) {
        ...
        } else if (number_of_queue_entries <= PHYS_MEM_SIZE) {
            tail->next = temp;
            tail = temp;
            number_of_queue_entries++;
        } else {
            printf("Should not be possible, please check \n");
        }
...
(because no input write value is given in 2.txt), this effectively does
